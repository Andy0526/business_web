...
import jieba
import jieba.posseg
jieba.load_userdict('C:/Python27/Lib/site-packages/jieba-0.37-py2.7.egg/jieba/userdict.txt') #Load user dictionary to increse segmentation accuracy
...
def ugc_quality_score(title, content):
    import pandas as pd
    term_frequcy =  pd.read_pickle('data/term_frequcy.pkl')
    bigram_frequecy = pd.read_pickle('data/bigram_frequecy.pkl')
    ...
    if length < 1:
        ...
        len_weight = (length + 0.001) * 1.0 / 2000
        if len_weight > 1.0: len_weight = 1
        gram_weight = (base - 0.0001) / 1.0
        sen_weight = (sen_cnt - 0.0001) / 50
        quality_score = gram_weight * 0.8 + len_weight * 0.1  + sen_weight * 0.1
